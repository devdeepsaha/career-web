...
app.config['SESSION_COOKIE_DOMAIN'] = '.pothoprodorshok.mooo.com'
...
# IMPORTANT: Configure login_manager BEFORE setting unauthorized_handler
...
# CORS - AFTER login_manager setup
    ...
    try:
        ...
        if current_user.is_authenticated and session_id:
            try:
                session_obj = ChatSession.query.filter_by(id=session_id, user_id=current_user.id).first()
                if session_obj:
                    user_message = ChatMessage(session_id=session_id, sender='user', text=current_user_message)
                    db.session.add(user_message)
                    
                    ai_message = ChatMessage(session_id=session_id, sender='ai', text=response.text)
    ...
    try:
        ...
        if current_user.is_authenticated and session_id:
            try:
                session_obj = ChatSession.query.filter_by(id=session_id, user_id=current_user.id).first()
                if session_obj:
                    user_message = ChatMessage(session_id=session_id, sender='user', text=question)
                    db.session.add(user_message)
